/**
 * Seeded 3D simplex noise (Stefan Gustavson's public-domain algorithm),
 * plus an fBM helper. Used on the CPU to displace procedural geometry so
 * every surface in the cell is organic rather than mathematically perfect.
 */
...
/** Deterministic PRNG (mulberry32) so the cell looks identical on every load. */
...
/** Shared instance — one seed for the whole cell so geometry is reproducible. */
export const cellNoise = new SimplexNoise3D(20240717);
